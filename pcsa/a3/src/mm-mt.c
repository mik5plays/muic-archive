...
// multithreaded version for task 4
...
// Task 4 - multithreading stuff
void* threaded_block_multiply(void* arg)
{
	ThreadArg* t = (ThreadArg*) arg;
	long i, j, k, i_block, j_block, k_block;

	for (i_block = t->start_row; i_block < t->end_row; i_block += BLOCK_SIZE) {
		for (j_block = 0; j_block < SIZEY; j_block += BLOCK_SIZE) {
			for (k_block = 0; k_block < SIZEY; k_block += BLOCK_SIZE) {
				for (i = i_block; i < i_block + BLOCK_SIZE && i < t->end_row; i++) {
					for (j = j_block; j < j_block + BLOCK_SIZE && j < SIZEY; j++) {
						long sum = huge_matrixC[i * SIZEY + j];
						for (k = k_block; k < k_block + BLOCK_SIZE && k < SIZEY; k++) {
							sum += huge_matrixA[i * SIZEY + k] * huge_matrixB[k * SIZEY + j];
						}
						huge_matrixC[i * SIZEY + j] = sum;
					}
...
void multiply_mt()
{
	...
	long rows = SIZEX / NUM_THREADS;

	for (int i = 0; i < NUM_THREADS; i++) {
		args[i].start_row = i * rows;
		args[i].end_row = (i == NUM_THREADS - 1) ? SIZEX : args[i].start_row + rows;
		pthread_create(&threads[i], NULL, threaded_block_multiply, &args[i]);
	}
...
int main()
{
	...
	clock_t s,t;
	// testing all 3 functions in mm-mt so need new variables 
	double total_in_base = 0.0;
	double total_in_your = 0.0;
	double total_in_mt = 0.0;
	double total_mul_base = 0.0;
	double total_mul_your = 0.0;
	double total_mul_mt = 0.0;
	...
	printf("[BMM] Total time taken during the load = %f seconds\n", total_in_your);

	s = clock();
	multiply();
	t = clock();
	total_mul_your += ((double)t-(double)s) / CLOCKS_PER_SEC;
	printf("[BMM] Total time taken during the multiply = %f seconds\n", total_mul_your);
	fclose(fin1);
	fclose(fin2);
	fclose(fout);
	free_all();
	...
	load_matrix();
	t = clock();
	total_in_mt += ((double)t-(double)s) / CLOCKS_PER_SEC;
	printf("[MT] Total time taken during the load = %f seconds\n", total_in_mt);
	...
	total_mul_mt += ((double)ts_end.tv_sec  - (double)ts_start.tv_sec)
						+ ((double)ts_end.tv_nsec - (double)ts_start.tv_nsec) / 1000000000.0;

	printf("[MT] Total time taken during the multiply = %f seconds\n", total_mul_mt);
