...
request_header: token ows t_colon ows text ows t_crlf {
	...
	if (parsing_request->header_count >= parsing_request->headers_capacity) {
        parsing_request->headers_capacity *= 2;
        parsing_request->headers = realloc(
            parsing_request->headers,
            sizeof(Request_header) * parsing_request->headers_capacity
        );
        if (!parsing_request->headers) {
            fprintf(stderr, "Failed to realloc headers\n");
            exit(1);
        }
    }
 ...
 * 2616.  All the best! ----- added some rules for when there might be not one header
 *
 */
...
headers:
  ...
  | headers request_header
    {
        YPRINTF("Parsed one header line.\n");
    };
